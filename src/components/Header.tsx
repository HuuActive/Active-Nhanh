import { ShoppingBag, Search, User, Menu, Zap, LogOut, Settings, ChevronDown, Package, Shield, X } from 'lucide-react';
...
export default function Header({ 
  ...
  return (
    <header className={`sticky top-0 w-full border-b border-brand-100 bg-white/95 backdrop-blur-md transition-all ${isMobileMenuOpen ? 'z-[110]' : 'z-40'}`}>
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
                ...
                <div className="p-6 border-b border-brand-50 bg-tiktok-black text-white">
                  ...
                  {user ? (
                    ...
                    <button 
                      ...
                      className="flex w-full items-center justify-center gap-2 rounded-xl bg-tiktok-cyan py-3 text-sm font-black text-tiktok-black transition-transform active:scale-95"
                    >
                      <User className="h-4 w-4" /> Đăng nhập ngay
                    </button>
                  )}
                </div>

                <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-2">
                  <div className="text-[10px] font-black tracking-widest text-brand-400 mb-2 px-2 uppercase">Điều hướng</div>
                  {['Sản phẩm', 'Bảng giá', 'Hướng dẫn', 'Liên hệ'].map((item) => (
                    <button
                      key={item}
                      onClick={() => {
                      ...
                      className="flex items-center gap-4 rounded-xl px-4 py-3 text-left font-bold text-brand-600 transition-colors hover:bg-brand-50 hover:text-tiktok-black"
                    >
                  ...
                  {user && (
                    <>
                      <div className="my-4 border-t border-brand-50" />
                      <div className="text-[10px] font-black tracking-widest text-brand-400 mb-2 px-2 uppercase">Cá nhân</div>
                      <button 
                        onClick={() => { setIsMobileMenuOpen(false); onMyOrdersClick(); }}
                        className="flex items-center gap-4 rounded-xl px-4 py-3 text-left font-bold text-brand-600 transition-colors hover:bg-brand-50 hover:text-tiktok-black"
                      >
                        <Package className="h-4 w-4 text-brand-300" /> Đơn hàng của tôi
                      </button>
                      
                      {isAdmin && (
                        <button 
                          onClick={() => { setIsMobileMenuOpen(false); onAdminClick(); }}
                          className="flex items-center gap-4 rounded-xl px-4 py-3 text-left font-bold text-brand-600 transition-colors hover:bg-brand-50 hover:text-tiktok-cyan"
                        >
                          <Shield className="h-4 w-4 text-tiktok-cyan" /> Quản trị hệ thống
                        </button>
                      )}
                      
                      <button 
                        onClick={() => { setIsMobileMenuOpen(false); logout(); }}
                        className="mt-2 flex items-center gap-4 rounded-xl px-4 py-3 text-left font-bold text-tiktok-magenta transition-colors hover:bg-red-50"
                      >
                ...
                <div className="p-6 bg-brand-50/50">
                   <div className="rounded-2xl bg-white border border-brand-100 p-4 shadow-sm">
                      ...
                      <div className="mt-3 flex gap-2">
                        <div className="h-1 w-full rounded-full bg-tiktok-cyan opacity-20" />
                        <div className="h-1 w-full rounded-full bg-tiktok-magenta opacity-20" />
                      </div>
                   </div>
                </div>
              </motion.div>
        ...
        <div className="flex items-center gap-2 sm:gap-4">
          ...
          {user ? (
            ...
            <button 
              onClick={login}
              className="tiktok-button flex items-center gap-2 text-xs"
            >
